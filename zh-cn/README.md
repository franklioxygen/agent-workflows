...
`agent-workflows` 帮助代理为项目初始化、功能开发、缺陷修复、代码评审、事故响应、重构和技术债清理选择合适流程。该库将工作流专属指导与共享的安全规则、预检和验证约定分离，使文档更易复用，也更容易维护，同时通过额外检查点帮助吸收模型基座静默降智带来的影响。
...
工作流也有助于缓冲模型基座静默降智带来的影响。如果底层模型在没有明显产品变化的情况下变得更不仔细、更不稳定或更不一致，工作流仍然会通过额外的检查点，尽量避免产出质量出现严重且隐蔽的下滑。

下图来自一次 6 任务的 Claude Code 探索性研究。完整统计协议（盲评、自举置信区间、配对置换检验配合 Holm–Bonferroni 校正、评分者间一致性等）详见 [evaluation/README.md](../evaluation/README.md)。

![实测结果对比](../assets/estimated-outcome-comparison.zh-cn.svg)

评分说明：所有分数都是越高越好。

指标说明：

- `TPR`：任务通过率——重复运行后的任务级平均通过率。
- `RP@k`：k 次可靠通过率——所有重复运行都通过的任务占比。
- `CPR`：干净通过率——同时满足任务通过、验证通过、无回归、无需返工的运行占比。
- `RFR`：无回归率——没有无关回归，且在声明了锁定评估检查时这些检查也通过的运行占比。
- `NRR`：无返工率——无需补救性修复的运行占比。

解读：

- `Task Pass Rate` 会提升，因为工作流改善的是执行纪律和验证，而不是模型本身的原始能力。
- `Reliable Pass@k` 会更明显地提升，因为工作流通过稳定的分诊、实现、验证顺序降低了运行波动。
- `Clean Pass Rate` 是头图里最严格的质量指标，因为一次运行必须任务通过、验证通过、没有无关回归，并且不需要修复性返工。
- `Regression-Free Rate` 和 `No-Rework Rate` 会改善，因为工作流通过基线捕捉、隐藏回归/评估检查与变更后重验证降低了出错率。
